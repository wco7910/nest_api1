...
import { KoreanUnitFiles } from './korean_unit_files.entity';
import { KoreanUnits } from './korean_units.entity';
import { BaseEntity } from './base.entity';

@Entity({ name: 'korean_unit_details', comment: '한글 단계(Unit) 상세 테이블' })
export class KoreanUnitDetails extends BaseEntity {
  ...
  @Column({ length: 50, comment: '모음', nullable: true })
  unit_vowel: string;

  @Column({ length: 50, comment: '모음 한글 발음', nullable: true })
  unit_speak: string;

  @Column({ length: 50, comment: '모음 영어 발음', nullable: true })
  unit_english: string;
  ...
  @Column({ length: 100, comment: '각 unit에 필요한 문장', nullable: true })
  ...
  @OneToMany(() => KoreanUnitFiles, (file) => file.unitDetails, {
    cascade: true,
  })
  files: KoreanUnitFiles[];
}
